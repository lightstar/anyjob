...
6. Стоит реализовать ряд модулей-рабочих общего пользования 'из коробки'. Например, такой, который
...
7. Все сообщения, отображаемые приложениями, сейчас только на английском языке. Неплохо бы сделать
...
8. Сейчас приложение slack требует явного указания типа задачи и списка узлов в тексте команды. Стоит добавить
возможность указывать группу, тип задачи и список узлов с помощью диалогов (правда, придется их показывать несколько
по очереди).

9. В веб-приложение стоит добавить поддержку ссылок, которые бы вели на частично созданную задачу, чтобы упростить
их создание.
