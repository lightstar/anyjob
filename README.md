...
**Задача (job)** - это сущность, имеющая конкретный тип (*type*), набор параметров (*params*)
и свойств (*props*). Все задачи создаются компонентом создателя (*creator*) и могут объединяться
во взаимосвязанные группы или наборы (*jobset*). Параметры и свойства имеют одинаковую структуру
(это список вида 'ключ-значение'), и разница лишь в том, что набор возможных параметров индивидуален для
...
**Узел (node)** - физический или виртуальный сервер, который обсуживает AnyJob.
...
через запятую, а также набор параметров и свойств в формате 'ключ=значение'.
...
один или несколько контроллеров (*controller*), каждый из которых выполняет собственную задачу.
Демон запускается на каждом узле с помощью bash-скрипта *rc.anyjobd*. На одном из узлов
должен быть запущен *глобальный (global)* контроллер, который осуществляет управление системой в целом,
в частности наборами задач, а на всех, где планируется запускать задачи - *рядовой (regular)*
контроллер.
...
в рамках системы. Работает внутри демона.
...
2. Сейчас создавать задачи можно только напрямую с их реальным типом, параметрами и свойствами.
Было бы неплохо при создании указывать некий абстрактный тип и параметры, которые уже в создателе
по каким-то правилам преобразовывались бы в реальную задачу или даже набор задач. Например, можно будет
...
3. Сейчас задача выполняется в отдельном процессе, причем для каждого запуска задачи создается новый
...
4. Конфигурация сейчас всегда грузится сразу и целиком. Вероятно, это не очень хорошо, если учесть ее
...
10. Сейчас все процессы-рабочие выполняются от имени *root*. Стоит реализовать возможность указывать
пользователя/группу, от имени которых они будут запускаться.

11. Все сообщения, отображаемые приложениями, сейчас только на английском языке. Неплохо бы сделать
...
12. Сейчас приложение slack требует явного указания типа задачи и списка узлов в тексте команды. Стоит добавить
возможность указывать группу, тип задачи и список узлов с помощью диалогов (правда, придется их показывать несколько
по очереди).

13. В веб-приложение стоит добавить поддержку ссылок, которые бы вели на частично созданную задачу, чтобы упростить
их создание.
