...
4. Сейчас при создании задачи она сразу же отправляется на выполнение. Было бы неплохо реализовать
отложенный запуск задач.

5. Сейчас как для хранилища данных, так и для очередей сообщений используется *redis*. И хотя он неплохо
...
6. Было бы неплохо ограничить выполнение задач с помощью каких-нибудь настраиваемых блокировок или
...
7. Стоит реализовать ряд модулей-рабочих общего пользования 'из коробки'. Например, такой, который
...
8. Все сообщения, отображаемые приложениями, сейчас только на английском языке. Неплохо бы сделать
...
9. Сейчас приложение slack требует явного указания типа задачи и списка узлов в тексте команды. Стоит добавить
возможность указывать группу, тип задачи и список узлов с помощью диалогов (правда, придется их показывать несколько
по очереди).

10. В веб-приложение стоит добавить поддержку ссылок, которые бы вели на частично созданную задачу, чтобы упростить
их создание.
