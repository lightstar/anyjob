...
sub continueDelayedWorkAction {
    ...
    if (scalar(@{$event->{works}}) != 1) {
        ...
        unless ($self->parentAddon->checkDelayedWorkAccess($build->{userId}, $action, $work)) {
            ...
            if (defined($build->{job})) {
                $response = $self->delayJob($build->{delay}, $build->{job}, $build->{userId}, $build->{responseUrl},
                    $build->{userName}, $work->{update});
            } else {
                $response = $self->updateDelayedWork($build->{delay}, $build->{userId}, $build->{responseUrl},
                    $build->{userName}, $work->{update});
            }
        } elsif ($action eq DELAY_ACTION_DELETE) {
            $response = $self->deleteDelayedWork($build->{delay}, $build->{userId}, $build->{responseUrl},
                $build->{userName}, $work->{update});
        }
    }
...
###############################################################################
# Update delayed work using data parsed from input arguments.
#
# Arguments:
#     delay       - hash with delay data generated by AnyJob::Creator::Parser module.
#     userId      - string user id.
...
sub updateDelayedWork {
    ...
    $self->debug('Update delayed work using slack app simple build by user \'' . $userId .
        '\' (\'' . $userName . '\') with response url \'' . $responseUrl . '\'' .
        (defined($updateCount) ? ', update count: ' . $updateCount : '') .
    ...
    $self->parent->updateDelayedWork($delay, undef, {
        creator      => 'slack',
        author       => $userName,
        observer     => 'slack',
        response_url => $responseUrl
