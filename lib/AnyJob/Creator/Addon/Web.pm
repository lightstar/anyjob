...
sub checkAuth {
    ...
    if (defined(my $cryptedPass = $config->{$user})) {
        return ($cryptedPass eq '<any>' or crypt($pass, $cryptedPass) eq $cryptedPass) ? 1 : 0;
    }
...
###############################################################################
# Generate one-time password to use for websocket connection authorization.
# Generated password will be valid for some limited amount of time (30 seconds).
# For details see https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm
#
# Arguments:
#     user - string user login.
# Returns:
#     string password.
#
sub generateOneTimePassword {
    my $self = shift;
    my $user = shift;

    unless (exists($self->{secretsByUser}->{$user})) {
        $self->{secretsByUser}->{$user} = mkpasswd();
    }

    return $self->{oath}->totp($self->{secretsByUser}->{$user});
}

###############################################################################
# Check one-time password previously generated by generateOneTimePassword method.
#
# Arguments:
#     user - string user login.
#     pass - string one-time password.
# Returns:
#     0/1 flag. If set, password is valid.
#
sub checkOneTimePassword {
    my $self = shift;
    my $user = shift;
    my $pass = shift;
    ...
    return $self->{oath}->totp($self->{secretsByUser}->{$user}) eq $pass ? 1 : 0;
}
